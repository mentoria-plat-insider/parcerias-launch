...
export default function CadastroParticipacao() {
  ...
  const [readinessReflection, setReadinessReflection] = useState("");
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (partnershipGoal.trim().length < 10 || readinessReflection.trim().length < 10) {
      toast.error("Responda às duas perguntas estratégicas antes de enviar.");
      return;
    }
  ...
  return <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6 sm:py-12"><section className="mx-auto w-full max-w-2xl"><a href="/" className="label-ed text-primary hover:underline">← Voltar à página inicial</a><div className="mt-5 overflow-hidden rounded-xl border border-border bg-card shadow-sm"><div className="border-b border-border p-6 sm:p-8"><p className="label-ed text-primary">Participação no evento</p><h1 className="font-display mt-3 text-3xl font-semibold">{current?.status === "rejected" ? "Revise seu cadastro" : "Cadastre-se para a Rodada"}</h1><p className="mt-3 max-w-xl text-sm leading-relaxed text-muted-foreground">Escolha o perfil que representa sua participação. O acesso ao painel acontece somente após a aprovação de um Administrador.</p>{current?.status === "rejected" && current.reviewNote && <p className="mt-4 rounded-md border border-destructive/25 bg-destructive/5 p-3 text-sm text-muted-foreground"><strong className="text-foreground">Retorno da operação:</strong> {current.reviewNote}</p>}</div>
        <form className="space-y-6 p-6 sm:p-8" onSubmit={handleSubmit}>
          ...
          <fieldset className="space-y-5 rounded-lg border border-primary/20 bg-primary/5 p-5"><legend className="label-ed px-1 text-primary">Antes de buscar uma parceria</legend><p className="-mt-2 text-sm leading-relaxed text-muted-foreground">Estas perguntas ajudam você a reconhecer o momento atual e chegar à conversa com uma intenção clara.</p><label className="block"><span className="label-ed text-foreground">O que você espera construir ou destravar com esta parceria? <span className="text-primary">*</span></span><textarea required minLength={10} rows={3} value={partnershipGoal} onChange={event => setPartnershipGoal(event.target.value)} className="mt-2 w-full resize-y rounded-md border border-input bg-background px-3 py-2.5 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/60" placeholder="Descreva um resultado concreto para esta Rodada" /></label><label className="block"><span className="label-ed text-foreground">Qual é o próximo passo que você realmente está pronto para assumir? <span className="text-primary">*</span></span><textarea required minLength={10} rows={3} value={readinessReflection} onChange={event => setReadinessReflection(event.target.value)} className="mt-2 w-full resize-y rounded-md border border-input bg-background px-3 py-2.5 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/60" placeholder="Reflita sobre disponibilidade, compromisso e capacidade de execução" /></label></fieldset><div className="rounded-md border border-primary/20 bg-primary/5 p-4 text-sm leading-relaxed text-muted-foreground"><ShieldCheck className="mr-2 inline size-4 text-primary" />Seus dados serão usados exclusivamente para identificar sua participação e viabilizar a operação da Rodada de Parcerias.</div>
